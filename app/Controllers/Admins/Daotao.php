...
class Daotao extends BaseController
{	
	var $control = 'daotao';
	var $title = "chương trình đào tạo";
	...
	{	
		...
		$datas = $this->DaotaoModels->select_array();
		$data = [
			'datas' => $datas,
	...
	public function add(){
		...
		if ($this->request->getPost()) {
				...
				$result = $this->DaotaoModels->add($data_post);
	...
	function delete($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$result = $this->DaotaoModels->delete_where(array('id'=>$id));
		if ($result) {
	...
	function edit($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$datas = $this->DaotaoModels->find_one(array('id'=>$id));
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
	
			$result = $this->DaotaoModels->edit($data_post,array('id'=>$id));
			if ($result) {
	...
	function checkglobals(){
		...
		$result = $this->DaotaoModels->edit($dataUpdate,array('id'=>$id));
		if ($result) {
	...
	function deleteAll(){
		...
		foreach($list_id_delete as $key => $val){	
			$result =$this->DaotaoModels->delete_where(array('id'=>$val));
			if ($result) {
