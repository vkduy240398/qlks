...
class Nguyennhan extends BaseController
{	
	var $control = 'nguyennhan';
	var $title = "Nguyên nhân";
	...
	{	
		...
		$datas = $this->NguyennhanModels->select_array();
		$data = [
			'datas' => $datas,
	...
	public function add(){
		...
		if ($this->request->getPost()) {
				...
				$result = $this->NguyennhanModels->add($data_post);
	...
	function delete($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$result = $this->NguyennhanModels->delete_where(array('id'=>$id));
		if ($result) {
	...
	function edit($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$datas = $this->NguyennhanModels->find_one(array('id'=>$id));
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
	
			$result = $this->NguyennhanModels->edit($data_post,array('id'=>$id));
			if ($result) {
	...
	function checkglobals(){
		...
		$result = $this->NguyennhanModels->edit($dataUpdate,array('id'=>$id));
		if ($result) {
	...
	function deleteAll(){
		...
		foreach($list_id_delete as $key => $val){	
			$result =$this->NguyennhanModels->delete_where(array('id'=>$val));
			if ($result) {
