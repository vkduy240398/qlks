...
class Posi extends BaseController
{	
	var $control = 'vitri';
	var $title = "Vị trí làm việc";
	...
	{	
		...
		$datas = $this->VitriModels->select_array();
		$data = [
			'datas' => $datas,
	...
	public function add(){
		...
		if ($this->request->getPost()) {
				...
				$result = $this->VitriModels->add($data_post);
	...
	function delete($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$result = $this->VitriModels->delete_where(array('id'=>$id));
		if ($result) {
	...
	function edit($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$datas = $this->VitriModels->find_one(array('id'=>$id));
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
	
			$result = $this->VitriModels->edit($data_post,array('id'=>$id));
			if ($result) {
	...
	function checkglobals(){
		...
		$result = $this->VitriModels->edit($dataUpdate,array('id'=>$id));
		if ($result) {
	...
	function deleteAll(){
		...
		foreach($list_id_delete as $key => $val){	
			$result =$this->VitriModels->delete_where(array('id'=>$val));
			if ($result) {
