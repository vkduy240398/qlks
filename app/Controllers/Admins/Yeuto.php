...
class Yeuto extends BaseController
{	
	var $control = 'yeuto';
	var $title = "Yếu tố";
	...
	{	
		...
		$datas = $this->YeutoModels->select_array();
		$data = [
			'datas' => $datas,
	...
	public function add(){
		...
		if ($this->request->getPost()) {
				...
				$result = $this->YeutoModels->add($data_post);
	...
	function delete($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$result = $this->YeutoModels->delete_where(array('id'=>$id));
		if ($result) {
	...
	function edit($id){
		if (!session('logged_in') == true) {return redirect()->to('/adminlogin.html');}
		$datas = $this->YeutoModels->find_one(array('id'=>$id));
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
	
			$result = $this->YeutoModels->edit($data_post,array('id'=>$id));
			if ($result) {
	...
	function checkglobals(){
		...
		$result = $this->YeutoModels->edit($dataUpdate,array('id'=>$id));
		if ($result) {
	...
	function deleteAll(){
		...
		foreach($list_id_delete as $key => $val){	
			$result =$this->YeutoModels->delete_where(array('id'=>$val));
			if ($result) {
